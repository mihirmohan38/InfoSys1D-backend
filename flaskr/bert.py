...
def prediction() : 
    ...
    db = get_db()
    posts = db.execute(
        "SELECT * FROM registereapt-get install python3-pipd WHERE username = ?" ,(username,)
    ).fetchall()
    ...
    attended_listdict = json.loads(rowList2json_activities(final_past))
    upcoming_listdict = json.loads(rowList2json_activities(final_future))
    ...
    return(upcoming_ids[recommended_indices])
